...
export default function AdminCategoryManager() {
  ...
  const handleOk = async () => {
    try {
      ...
      if (editMode && editingId) {
        await axios.patch(`/api/admin/category/${editingId}`, values);
      ...
      message.error(
        editMode ? "แก้ไขหมวดหมู่ไม่สำเร็จ" : "เพิ่มหมวดหมู่ไม่สำเร็จ"
      );
    }
  };
  ...
  return (
    ...
    <Modal
      title={editMode ? "แก้ไขหมวดหมู่สินค้า" : "เพิ่มหมวดหมู่สินค้า"}
      open={modalOpen}
      onOk={handleOk}
      onCancel={() => {
        setModalOpen(false);
